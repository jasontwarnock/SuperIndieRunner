...
class GameHUD: SKSpriteNode, HUDDelegate {
    ...
    init(with size: CGSize) {
        ...
        for i in 0..<3 {
            ...
            emptySlot.position = CGPoint(x: -superCoinCounter.size.width/2 + emptySlot.size.width/3 + CGFloat(i)*superCoinCounter.size.width/3 + superCoinCounter.size.width*0.05, y: superCoinCounter.frame.midY)
